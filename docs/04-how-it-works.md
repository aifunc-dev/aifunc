...
3. **Compile & wrap (core mechanism)**: To avoid issues that traditional bundlers (like Webpack/Vite) or language runtimes may have when loading raw data files, the CLI no longer outputs plain config files separately. Instead, it **wraps API definitions, prompt templates, model parameters, etc. using your target language**.
   ...
   * Java projects get `.java` files containing static `Map<String,Object>` initializers.
   * C# projects get `.cs` files containing object/dictionary initializers.
...
The project is divided into a cache area (not committed to Git) and a compiled artifacts area (committed to Git). Examples for TypeScript, Python, Go, Java, and C#:
...
│
└── aifunc/                              ← Compiled artifact output directory (commit to Git)
    ├── summarize/                       ← AI function package (package aifunc.summarize)
    │   ├── Summarize.java               ← Function entry point (static method, returns CompletableFuture)
    │   ├── SummarizeTypes.java          ← SummarizeInput / SummarizeOutput POJOs
    │   ├── SummarizeAifunc.java         ← Compiled artifact (Map<String,Object> literal)
    │   └── SummarizeAifuncMock.java     ← Mock data (Map literal, null when no mock.json)
    │
    ├── AIFuncConfig.java                ← Public config (package aifunc)
    ├── AIFuncException.java             ← Public exception (package aifunc)
    └── _engine/
        └── java/
            └── v0_1_0/
                  ├── Json.java          ← Zero-dep JSON parser/serializer
                  ├── Types.java
                  ├── Runtime.java       ← executeAsync() entry point
                  ├── Validator.java
                  ├── Prompt.java
                  ├── Request.java
                  ├── Mock.java
...
No build tool required — generated files are plain Java sources with zero external dependencies. Place `aifunc/` next to your application code (or add it as a source root) and compile with `javac`.
...
│
└── aifunc/                              ← Compiled artifact output directory (commit to Git)
    ├── summarize/                       ← AI function package (namespace Aifunc.Summarize)
    │   ├── Summarize.cs                 ← Function entry point (SummarizeAsync)
    │   ├── SummarizeTypes.cs            ← SummarizeInput / SummarizeOutput records
    │   ├── SummarizeAifunc.cs           ← Compiled artifact (dictionary literal)
    │   └── SummarizeAifuncMock.cs       ← Mock data (dictionary literal, null when no mock.json)
    │
    ├── AIFuncConfig.cs                  ← Public config (namespace Aifunc)
    ├── AIFuncException.cs               ← Public exception (namespace Aifunc)
    └── _engine/
        └── csharp/
            └── v0_1_0/
...
No NuGet packages needed — generated files are plain C# sources with zero external dependencies. Place `aifunc/` next to your application code (or add it as a source root) and run with `dotnet run`.

> [!NOTE]
> Files within packages use different naming conventions depending on the target language (e.g., TS uses `.` separators, Python and Go use `_` separators, Java and C# use PascalCase). All code dependencies are self-contained within the generated directory.
...
|:---|:---|
| `entry` (e.g., `index.ts`, `summarize.go`, `Summarize.java`, `Summarize.cs`) | **Function entry point**: Creates and exports the AI function instance. Your business code imports this directly. |
| `types` (e.g., `.types.ts`, `_types.go`, `SummarizeTypes.java`, `SummarizeTypes.cs`) | **Interface file**: Input and output structure type definitions, providing strong typing support. |
| `aifunc` (e.g., `.aifunc.ts`, `_aifunc.go`, `SummarizeAifunc.java`, `SummarizeAifunc.cs`) | **Core artifact**: Merged result of prompt template, API spec, and model config, wrapped in the target language to ensure safe runtime loading. |
| `mock` (e.g., `.mock.ts`, `_mock.py`, `SummarizeAifuncMock.java`, `SummarizeAifuncMock.cs`) | **Mock data**: Input-to-output mapping data, also wrapped in the target language, used for offline test mode. (Go embeds mock data directly in the entry file.) |
...
Your application code
  ...
  ▼
Generated function entry (aifunc/summarize/index.ts | summarize.go | Summarize.java | Summarize.cs)
...
| Field | Description |
|:---|:---|
| `language` | Target language, determines the language wrapper format and Engine SDK language (currently supports `typescript`, `python`, `go`, `java`, `csharp`) |
