...
流式示例见：[TypeScript](./examples/typescript/chat-stream) / [Python](./examples/python/chat-stream) / [Go](./examples/go/chat-stream) / [Java](./examples/java/chat-stream) / [C#](./examples/csharp/chat-stream) — 另见 [all-packages-stream](./examples/typescript/all-packages-stream)
...
**流式输出** — 包可通过 `"x-delivery-mode": "stream"` 按 token 流式返回纯文本，用各语言原生异步迭代器 / channel 消费。
