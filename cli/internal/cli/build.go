...
func buildOneWithEngine(ws *workspace.Workspace, cfg types.AifuncConfig, name string, lock types.LockFile) error {
	...
	case "go":
		goModule, _ := fileutil.ReadGoModule(filepath.Join(ws.Root, "go.mod"))
		hasMock := packageHasMock(pkgPath)
		if err := golang.Generate(artifact, pkgOutputDir, hasMock, engineVersion, cfg, goModule); err != nil {
			fmt.Println("failed")
		...
		if err := writeGoMockFile(pkgPath, name, pkgOutputDir); err != nil {
			fmt.Println("failed")
...
func writeGoMockFile(pkgPath, name, pkgOutputDir string) error {
	mockSrc := filepath.Join(pkgPath, "mock.json")
	if _, err := os.Stat(mockSrc); os.IsNotExist(err) {
		return nil
	}
	...
	if !ok {
		return fmt.Errorf("mock.json must be a JSON object")
	}
	encoded := golang.ToGoMap(mockMap, 0)
	safeName := strings.TrimPrefix(name, "@")
	safeName = strings.ReplaceAll(safeName, "/", "__")
	safeName = strings.ReplaceAll(safeName, "-", "_")
	pkgName := safeName
	...
	fmt.Fprintf(&content, "var %sMockData any = %s\n", safeName, encoded)
	mockDst := filepath.Join(pkgOutputDir, safeName+"_mock.go")
	return os.WriteFile(mockDst, []byte(content.String()), 0644)
}
