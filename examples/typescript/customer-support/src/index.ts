...
async function main() {
  ...
  const intentResult = await recognizeIntent(config, { text: message, intents: INTENTS } as RecognizeIntentInput);
  ...
  switch (intent) {
    case 'query_order': {
      const info = await extractJson(config, { text: message, fields: [
